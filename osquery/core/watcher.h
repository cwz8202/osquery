...
class Watcher : private boost::noncopyable {
 public:
  ...
  static void setWorker(const std::shared_ptr<PlatformProcess>& child) { instance().worker_ = child; }

  /// Setter for an extension process.
  static void setExtension(const std::string& extension, const std::shared_ptr<PlatformProcess>& child);
 ...
 private:
  ...
  Watcher()
      : worker_(std::make_shared<PlatformProcess>()), worker_restarts_(0), lock_(mutex_, std::defer_lock) {}
...
class WatcherWatcherRunner : public InternalRunnable {
 public:
  explicit WatcherWatcherRunner(const std::shared_ptr<PlatformProcess>& watcher) : watcher_(watcher) {}
